  ...
  return (
    <div className="custom-option">
      <div className="option-content">
        <input type="checkbox" checked={isSelected} onChange={() => selectOption(option)} className="option-checkbox" />
    ...
    if (selectedType === "BLOC") {
      ...
      try {
        const results = await Promise.all(
          selectedStairs.map((s) => fetch(`${apiUrl}/stair?stairId=${s.value}`).then((r) => r.json())),
        )
